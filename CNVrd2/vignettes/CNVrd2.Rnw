...
@ 
From the results of the LD analysis, \emph{rs117435514} is the best tagSNP for duplications: 0$\%$ (zero samples), 2.78$\%$ (one sample) and 86.7$\%$ (13 samples) of deleted, normal and duplicated samples have this SNP (adjusted p-value = 7.1e-08, $r^2$ = 0.66).

We can write out all these tagSNP results:
...
The output above provides evidence that the European population has weak tag SNPs for CNV of the CCL3L1 gene. To better understand tag SNPs inside the European population, we can apply the test to small populations in this large population.
...
The output above provides evidence that 
...
To write these results to csv files for populations, we can use the code below:
...
Note: All TagSNP results for DEFB103A, CCL3L1 and FCGR3B CNV will be uploaded on \url{https://github.com/hoangtn/CNVrd2}
