...
\section{Note}
If we use the option \emph{entireGene = TRUE} in the step \emph{segmentation} then the pipeline will not refine the segmentation results (the results will be the same as the pipeline used in \cite{nguyen2013cnvrd}).
